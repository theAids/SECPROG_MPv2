    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (session != null) {
            ...
            if (acl.getEDIT_CD() == 1) {
                ...
                CDBean db = new CDBean();
                ProductLogBean plb = new ProductLogBean();
              
                //get which row
                String orig = request.getParameter("origCDtitle");
                pb.setProductID(pi.getProductByTitle(orig).getProductID());
                //checks for changes
                String title = request.getParameter("editCDtitle");
                if (title.length() > 0) {
                    pi.editProductByTitle(pb.getProductID(), title);
                    plb.setActivity("E- CD");
                    pli.addProductLog(user, pb, plb);
                }
                String summary = request.getParameter("editCDSummary");
                if (summary.length() > 0) {
                    pi.editProductBySummary(pb.getProductID(), summary);
                    plb.setActivity("E- CDSUM");
                    pli.addProductLog(user, pb, plb);
                }
                String price = request.getParameter("editCDPrice");
                if (price.length() > 0) {
                    pi.editProductByPrice(pb.getProductID(), Float.parseFloat(price));
                    plb.setActivity("E- CDPRi");
                    pli.addProductLog(user, pb, plb);
                }
                String stock = request.getParameter("editCDStock");
                if (stock.length() > 0) {
                    pi.editProductByStock(pb.getProductID(), Float.parseFloat(stock));
                    plb.setActivity("E- CDSto");
                    pli.addProductLog(user, pb, plb);
                }
                String status = request.getParameter("editCDPstatus");
                if (status.length() > 0) {
                    pi.editProductByStatus(pb.getProductID(), status);
                    plb.setActivity("E- CDSta");
                    pli.addProductLog(user, pb, plb);
                }
                String artist = request.getParameter("editCDArtist");
                if (artist.length() > 0) {
                    pi.editCDByArtist(pb.getProductID(), artist);
                    plb.setActivity("E- CDArt");
                    pli.addProductLog(user, pb, plb);
                }
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet editCD</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet editCD at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
